...
module N_ary_function = struct
  ...
  let n_ary_function_expr ext x =
    ...
    AST.wrap_desc Expression ~attrs:[] ~loc:x.pexp_loc @@
    AST.make_jane_syntax Expression feature suffix x

  let expr_of ~loc (params, constraint_, body) =
    (* See Note [Wrapping with make_entire_jane_syntax] *)
    AST.make_entire_jane_syntax ~loc Expression feature (fun () ->
      let body =
        match body with
        | Pfunction_body body ->
...
module Expression = struct
  ...
  let expr_of ~loc t =
    match t with
    | Jexp_comprehension t -> Comprehensions.expr_of ~loc t
    | Jexp_immutable_array t -> Immutable_arrays.expr_of ~loc t
    | Jexp_n_ary_function t -> N_ary_function.expr_of ~loc t
end
