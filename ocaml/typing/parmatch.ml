...
let pats_of_type ?(always=false) env ty =
  ...
  | Ttuple tl ->
      [make_pat (Tpat_tuple (List.map (fun (lbl, _) -> lbl, omega) tl)) ty env]
  | _ -> [omega]
    ...
    let rec loop pat =
      ...
      | Tpat_tuple lst ->
        if List.for_all (fun (label, _) -> Option.is_none label) lst then
          ...
          in
          mkpat ~attrs:ppat.jane_syntax_attributes ppat.desc
...
| Tpat_array (_, ps) | Tpat_construct (_, {cstr_tag=Extension _}, ps, _)->
