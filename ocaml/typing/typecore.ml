...
let rec type_approx_fun env loc label spato ty_expected ~first ~in_function =
...
and type_approx env sexp ty_expected =
  ...
  let rec loop env params c body ty_expected ~in_function ~first =
    ...
    *)
    ...
    | Pparam_val (label, _, pat) :: params ->
        let ty_res =
          type_approx_fun env loc label (Some pat) ty_expected
            ~first ~in_function
        in
    ...
    | [] ->
        ...
        *)
        ...
        | Pfunction_cases ({pc_rhs = e} :: _, _, _) ->
            let ty_res =
              type_approx_fun env loc Nolabel None ty_expected
                ~in_function ~first
            in
...
and type_expect_jane_syntax
  ...
  | Jexp_n_ary_function n_ary_expr ->
      type_n_ary_function
        ~loc ~env ~expected_mode ~ty_expected ~explanation ~attributes
        n_ary_expr
